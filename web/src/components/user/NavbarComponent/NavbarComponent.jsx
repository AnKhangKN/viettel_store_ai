            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-200 cursor-pointer flex-shrink-0 ${
                    isActive
                      ? 'bg-red-50 text-[#EE0033] dark:bg-red-950/70 dark:text-red-400 dark:border dark:border-red-800/60 shadow-xs'
                      : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80'
                  }`}
                >
                  <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-[#EE0033] dark:text-red-400' : 'text-slate-400 dark:text-slate-400'}`} />
